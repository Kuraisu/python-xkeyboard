...
// 2015-05-03 Aleksey Yermakov added code to retrieve caps lock, scroll lock and num lock states
...
bool XKeyboard::getModState(XKLockModifier mod) const
{
    Bool state;
    ...
    switch(mod) {
        case XKLMCapsLock:
            modAtom = XInternAtom(_display, "Caps Lock", False);
            break;
        case XKLMNumLock:
            modAtom = XInternAtom(_display, "Num Lock", False);
            break;
        case XKLMScrollLock:
            modAtom = XInternAtom(_display, "Scroll Lock", False);
            break;
        default:
            modAtom = None;
            break;
    ...
    XkbGetNamedIndicator(_display, modAtom, NULL, &state, NULL, NULL);
...
bool XKeyboard::capsLockState() const
{
    return getModState(XKLMCapsLock);
}

bool XKeyboard::numLockState() const
{
    return getModState(XKLMNumLock);
}

bool XKeyboard::scrollLockState() const
{
    return getModState(XKLMScrollLock);
}
